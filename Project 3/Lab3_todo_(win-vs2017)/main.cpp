...
const char lollipopTexFile[] = "images/lollipop.jpg";
const char stickTexFile[] = "images/stick.jpg";
...
void MakeReflectionImage( void )
{
    ...
    // STEP 2: Sets up the correct view volume for imaginary viewpoint.
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1], TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0], eyePos[2] - TABLETOP_Z + 0.001, eyePos[2] - TABLETOP_Z + SCENE_RADIUS);
    // STEP 3: Sets up the imaginary viewpoint.
    // STEP 3: Sets up the imaginary viewpoint.
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(eyePos[0], eyePos[1], 2 * TABLETOP_Z - eyePos[2], eyePos[0], eyePos[1], 2 * TABLETOP_Z - eyePos[2] + 1, 1.0, 0.0, 0.0);
    // STEP 4: Sets up the light source positions in the world space.
...
// This texture object is for the lollipop texture map.

    glGenTextures( 1, &lollipopTexObj );
    glBindTexture( GL_TEXTURE_2D, lollipopTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

    if ( ReadImageFile( lollipopTexFile, &imageData,
                        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        fprintf( stderr, "Error: Texture image is not in RGB format.\n" );
...
// This texture object is for the stick texture map.

    glGenTextures( 1, &stickTexObj );
    glBindTexture(GL_TEXTURE_2D, stickTexObj );
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(stickTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
void DrawGiantLollipop(void)
{
    ...
    double handle_radius = 0.04;
    double handle_length = 1.0;
    double distance_from_table = 0.3;

    // Stick

    GLfloat matAmbient0[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat matDiffuse0[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat matSpecular0[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat matShininess0[] = { 128.0 };
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, matAmbient0);
    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, matDiffuse0);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, matSpecular0);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, matShininess0);

    glPushMatrix();
    glTranslated(-0.5, 1, distance_from_table + TABLETOP_Z + handle_length / 2);
    glRotated(45.0, 1.0, 0.0, 0.0);

    glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    glBindTexture( GL_TEXTURE_2D, stickTexObj );

    // Disable back-face culling to render cylinder correctly.
    glDisable(GL_CULL_FACE);
    
    int no_partitions = 45;

    // Create lollipop stick in the form of a closed cylinder.

    // Surrounding surface.
    for (int i = 0; i <= 2 * no_partitions; i = i + 2) {
        glBegin(GL_POLYGON);

        glNormal3d(sin(PI * i / no_partitions), cos(PI * i / no_partitions), 0.5 * handle_length);
        glTexCoord2f((float) i / (float) (2 * no_partitions), 1.0);
        glVertex3d(handle_radius * sin(PI * i / no_partitions), handle_radius * cos(PI * i / no_partitions), 0.5 * handle_length + 0.1);
        
        glNormal3d(sin(PI * i / no_partitions), cos(PI * i / no_partitions), -0.5 * handle_length);
        glTexCoord2f((float) i / (float) (2 * no_partitions), -1.0);
        glVertex3d(handle_radius * sin(PI * i / no_partitions), handle_radius * cos(PI * i / no_partitions), -0.5 * handle_length);
        
        glNormal3d(sin(PI * (i + 2) / no_partitions), cos(PI * (i + 2) / no_partitions), -0.5 * handle_length);
        glTexCoord2f((float) (i + 2) / (float) (2 * no_partitions), -1.0);
        glVertex3d(handle_radius * sin(PI * (i + 2) / no_partitions), handle_radius * cos(PI * (i + 2) / no_partitions), -0.5 * handle_length);
        
        glNormal3d(sin(PI * (i + 2) / no_partitions), cos(PI * (i + 2) / no_partitions), 0.5 * handle_length);
        glTexCoord2f((float) (i + 2) / (float) (2 * no_partitions), 1.0);
        glVertex3d(handle_radius * sin(PI * (i + 2) / no_partitions), handle_radius * cos(PI * (i + 2) / no_partitions), 0.5 * handle_length + 0.1);

        glEnd();
    }

    // Bottom face, without texture mapping (intended).
    ...
    // Top face, does not matter much, without texture mapping (intended).
    glNormal3d(0.0, 0.0, 1.0);
    glBegin(GL_POLYGON);
    for (int i = 0; i <= 2 * no_partitions; i = i + 2) {
        glVertex3d(handle_radius * sin(PI * i / no_partitions), handle_radius * cos(PI * i / no_partitions), 0.5 * handle_length + 0.1);
    ...
    glTranslated(0.0, 0.0, handle_length / 2 + sphere_radius);

    // Lollipop

    GLfloat matAmbient1[] = { 0.7, 0.5, 0.2, 1.0 };
    GLfloat matDiffuse1[] = { 0.7, 0.5, 0.2, 1.0 };
    ...
    // Create lollipop candy in the form of a sphere.
    for (int i = 0; i < 2*no_partitions; i++) {
        for (int j = 0; j < 2*no_partitions; j++) {
            glBegin(GL_POLYGON);

            glNormal3d(sin(i * PI / no_partitions) * cos(j * PI / no_partitions), cos(i * PI / no_partitions) * cos(j * PI / no_partitions), sin(j * PI / no_partitions));
            glTexCoord2f((float) i / (float) (2 * no_partitions), (float) j / (float) (2 * no_partitions));
            glVertex3d(sphere_radius * sin(i * PI / no_partitions) * cos(j * PI / no_partitions), sphere_radius * cos(i * PI / no_partitions) * cos(j * PI / no_partitions), sphere_radius * sin(j * PI / no_partitions));
            
            glNormal3d(sin((i + 1) * PI / no_partitions) * cos(j * PI / no_partitions), cos((i + 1) * PI / no_partitions) * cos(j * PI / no_partitions), sin(j * PI / no_partitions));
            glTexCoord2f((float) (i + 1) / (float) (2 * no_partitions), (float) j / (float) (2 * no_partitions));
            glVertex3d(sphere_radius * sin((i + 1) * PI / no_partitions) * cos(j * PI / no_partitions), sphere_radius * cos((i + 1) * PI / no_partitions) * cos(j * PI / no_partitions), sphere_radius * sin(j * PI / no_partitions));
            
            glNormal3d(sin((i + 1) * PI / no_partitions) * cos((j + 1) * PI / no_partitions), cos((i + 1) * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sin((j + 1) * PI / no_partitions));
            glTexCoord2f((float) (i + 1) / (float) (2 * no_partitions), (float) (j + 1) / (float) (2 * no_partitions));
            glVertex3d(sphere_radius * sin((i + 1) * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sphere_radius * cos((i + 1) * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sphere_radius * sin((j + 1) * PI / no_partitions));
            
            glNormal3d(sin(i * PI / no_partitions) * cos((j + 1) * PI / no_partitions), cos(i * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sin((j + 1) * PI / no_partitions));
            glTexCoord2f((float) i / (float) (2 * no_partitions), (float) (j + 1) / (float) (2 * no_partitions));
            glVertex3d(sphere_radius * sin(i * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sphere_radius * cos(i * PI / no_partitions) * cos((j + 1) * PI / no_partitions), sphere_radius * sin((j + 1) * PI / no_partitions));
            glEnd();
        }
    }
