...
void DrawOneCar( float bodyColor[3] )
{
    ...
    //The size ratio of all part has been planned before implementing.
    //All size are proportional to CAR_LENGTH, CAR_WIDTH, CAR_HEIGHT.
    // Middle part
    glPushMatrix();
    glTranslatef(0.0, 0.0, CAR_HEIGHT / 2);
    glScalef((CAR_LENGTH / 2) / CAR_HEIGHT, (2 * CAR_WIDTH / 3) / CAR_HEIGHT, 1.0);
    glutSolidCube(CAR_HEIGHT);
    glPopMatrix();

    //Rear part
    glPushMatrix();
    glTranslatef((-3) * CAR_LENGTH / 8, 0.0, CAR_HEIGHT / 4);
    glScalef((CAR_LENGTH / 4) / (CAR_HEIGHT / 2), (2 * CAR_WIDTH / 3) / (CAR_HEIGHT / 2), 1.0);
    glutSolidCube(CAR_HEIGHT / 2);
    glPopMatrix();
    ...
    // Left front tyre
    glPushMatrix();
    glTranslatef(CAR_LENGTH / 4, (-5) * CAR_WIDTH / 12, CAR_HEIGHT / 4);
    glRotatef(90.0, 1.0, 0.0, 0.0);
    glutSolidTorus(CAR_WIDTH / 12, (CAR_HEIGHT / 4) - (CAR_WIDTH / 12), 36, 36);
    glPopMatrix();
...
void DrawAllCars( void )
{
    for ( int i = 0; i < NUM_CARS; i++ )
    {
        //****************************
        // WRITE YOUR CODE HERE.
        CarType current_car = car[i];
        //Take out the angle of vector v.
        double angle_0 = atan(current_car.xzAxis[1] / current_car.xzAxis[0]) * 180 / PI;
        //Steps to rotate about vector v.
        glPushMatrix();
        glRotatef(-angle_0, 0.0, 1.0, 0.0);
        glRotatef(current_car.rotAngle, 1.0, 0.0, 0.0);
        glRotatef(angle_0, 0.0, 1.0, 0.0);
        //Rotate so that the z azis is now the vector from center to car position.
        glRotatef(current_car.angularPos - 90, 1.0, 0.0, 0.0);
        //Translate to the car position.
        glTranslatef(0.0, 0.0, PLANET_RADIUS);
        glRotatef(90.0, 0.0, 0.0, 1.0);
        //Draw car
        DrawOneCar(current_car.bodyColor);
        glPopMatrix();
...
void MyDisplay( void )
{
    ...
    //***********************************************************************
    //Calculate camera frame origin coordinate and use it with glueLookAt()
    double cam_y = eyeDistance * sin(eyeLatitude * PI / 180);
    double cam_zx = eyeDistance * cos(eyeLatitude * PI / 180);
    double cam_z = cam_zx * cos(eyeLongitude * PI / 180);
    double cam_x = cam_zx * sin(eyeLongitude * PI / 180);
