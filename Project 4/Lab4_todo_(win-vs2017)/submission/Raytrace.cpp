...
// Compute for shadow if hasShadow is true.
    ...
    for (PointLightSource ptLight : scene.ptLights) {
        Vector3d L = ptLight.position - nearestHitRec.p;
        double L_len = L.length();
        L = L / L.length();
        if (hasShadow) {
            bool hitShadow = false;
            for (const auto& surface : scene.surfaces)
            {
                bool hasHitObject = surface->shadowHit(Ray(nearestHitRec.p, L), DEFAULT_TMIN, L_len);
                if (hasHitObject) {
                    hitShadow = true;
                    break;
                }
            ...
            if (!hitShadow) {
                result += computePhongLighting(L, N, V, nearestHitRec.material, ptLight);
            }
        }
        else {
            result += computePhongLighting(L, N, V, nearestHitRec.material, ptLight);
...
// Add to result the reflection of the scene.
    ...
    if (reflectLevels > 0) {
        ...
        Ray reflectedRay = Ray(nearestHitRec.p, R);
        Color reflectedColor = TraceRay(reflectedRay, scene, reflectLevels - 1, hasShadow);
        result += nearestHitRec.material.k_rg * reflectedColor;
    }
